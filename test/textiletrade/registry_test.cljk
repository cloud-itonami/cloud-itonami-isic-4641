(ns textiletrade.registry-test
  (:require [clojure.test :refer [deftest is]]
            [textiletrade.registry :as r]))

;; The textile-trading domain checks (credit-clearance, contract-on-file,
...
(deftest dispatch-assigns-dispatch-number
  (let [result (r/register-dispatch-record "to-1" "USA" 7)]
    ...
    (is (= (get-in result ["record" "textile_order_id"]) "to-1"))
    (is (= (get-in result ["record" "kind"]) "textile-dispatch-draft"))
...
(deftest invoice-assigns-invoice-number
  (let [result (r/register-invoice-record "to-1" "USA" 7)]
    ...
    (is (= (get-in result ["record" "textile_order_id"]) "to-1"))
    (is (= (get-in result ["record" "kind"]) "textile-invoice-draft"))
    (is (= (get-in result ["record" "immutable"]) true))))
