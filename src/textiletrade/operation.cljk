(ns textiletrade.operation
  "OperationActor -- one textile/apparel/footwear-wholesale operation =
  one supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (TextileTradeAdvisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Textile Trading Governor
  ...
  One graph run = one textile/apparel/footwear-wholesale operation
  (intake -> advise -> govern -> decide -> commit | hold | approval). No
  unbounded inner loop -- each operation is auditable and checkpointed. A
  textile-order's life is advanced by MANY ops (intake / verify /
  dispatch / settle), each its own independent run.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human operator (trading supervisor / warehouse
  operator). The approver resumes with `{:approval {:status :approved}}`
  (or :rejected). `:delivery/dispatch`/`:invoice/settle` ALWAYS reach
  this node when the governor is clean -- see `textiletrade.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [textiletrade.textiletradeadvisor :as textiletradeadvisor]
...
(defn build
  ...
  `textiletrade.store/Store`).
  opts:
    :advisor      -- a `textiletrade.textiletradeadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (textiletradeadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TextileTradeAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (textiletradeadvisor/-advise advisor store request)]
            {:proposal p :audit [(textiletradeadvisor/trace request p)]})))

      ;; Textile Trading Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
